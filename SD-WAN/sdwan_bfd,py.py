...
print(json.dumps(device_response["data"], indent=2))
# Loop through the returned list of device data and print BFD sessions
for device in device_response["data"]:
    print(f"The device hostname is {device['host-name']} at site ID {device['site-id']}")
    try:
        print(f"The BFD session no count is {device['bfdSessionsUp']}")
    except KeyError:
        print("No BFD Sessions")
    print("*" * 40)