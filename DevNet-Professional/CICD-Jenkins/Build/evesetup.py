...
def api_login():
    ...
    url = "https://192.168.1.189/api/auth/login"
...
def query_api(url, time_stamp, cookie):
    headers = {
        ...
        "Referer": "https://192.168.1.189/legacy/",
        "Accept-Language": "en-US,en;q=0.9",
        "Content-type": "application/json",
    }
    api_url = "https://192.168.1.189/api/"
    full_url = api_url + url
    logging.info(f"URL:{full_url}")
    nodes = requests.get(url=full_url, headers=headers, cookies=cookie, verify=False)
    response = nodes.json()
    return response
...
def main():
    ...
    parser.add_argument(
        "-t", "--topology", type=str, dest="topo", help="Provide topology name."
    )
    parser.add_argument(
        "-n", "--nodes", dest="nodes", action="store_true", help="List nodes."
    )
    parser.add_argument(
        "-u",
        "--up",
        dest="start",
        action="store_true",
        help="Provide lab name to start",
    )
    parser.add_argument(
        "-d",
        "--down",
        dest="stop",
        action="store_true",
        help="Provide lab name to stop",
    )
    parser.add_argument(
        "-a", "--all", dest="all_labs", action="store_true", help="List all labs."
    )
    parser.add_argument(
        "-w", "--wipe", dest="wipe", action="store_true", help="Wipe all nodes."
    )
