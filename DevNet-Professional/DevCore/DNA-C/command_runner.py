...
devices = requests.get(
    url=f"{base_url}{device_endpoint}", headers={"X-Auth-Token": token}, verify=False
).json()
...
task_response = requests.get(
    url=f"{base_url}intent/api/v1/task/{command_response['response']['taskId']}",
    headers={"X-Auth-Token": token},
    verify=False,
).json()
...
file_response = requests.get(
    url=f"{base_url}intent/api/v1/file/{task_response['response']['progress']['fileId']}",
    headers={"X-Auth-Token": token},
    verify=False,
).json()
