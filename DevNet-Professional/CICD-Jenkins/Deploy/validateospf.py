...
ip_address = ["192.168.1.122", "192.168.1.110"]
...
def test_ospf():
    ...
    for ip in ip_address:
        ...
        router_id = json.loads(routerid_resp.text)[
            "Cisco-IOS-XE-ospf-oper:ospf-instance"
        ][0]["router-id"]
        print(f"Retrieved router-id {router_id} for host {ip}")

        new_url = f"{url}=address-family-ipv4,{router_id}/ospf-area=0/ospf-interface/"
        interfaces = requests.get(
            url=new_url, headers=headers, auth=("admin", "Pa55word"), verify=False
        ).json()["Cisco-IOS-XE-ospf-oper:ospf-interface"]
        ospf_state = False
